...
- **Proxy-unaware tools fail closed**, not open -- an availability cost, deliberately paid until transparent interception lands. Kernel-level syscall/file observation is future work, and file/tool governance today is cooperative -- labeled `self-reported`, exactly as the model demands.
