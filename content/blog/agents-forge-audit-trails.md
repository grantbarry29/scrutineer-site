...
Here's a question for anyone running autonomous agents. If a prompt-injected agent exfiltrated a credential from your environment last week, would you know?
...
Many agent guardrails I've seen fall into one of three patterns. Each has real, widely used implementations; each is bypassable by the thing it's meant to govern.
...
I know the in-pod version well because I initially built it. Scrutineer's first enforcement tier was cooperative sidecars in the agent pod. A DNS proxy and a tool gateway that passed every demo. Then I wrote the attacks against my own design: the agent shares that pod's network namespace, so a raw socket skips the DNS proxy entirely, and a tight loop can starve the reporter until the record is whatever happened to survive. I deleted the tier. If a control can't survive a hostile occupant of its own pod, calling it "governance" just moves the honor system one process over.
...
For each session, the controller provisions a **per-session Envoy proxy in its own pod**, with its own ServiceAccount and network namespace, owner-referenced to the session and torn down with it. The agent pod gets standard `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` environment pointing at that proxy. Envoy terminates HTTP and tunnels HTTPS and any other TCP the client sends via `CONNECT`.
...
- **Coverage is HTTP/S plus `CONNECT`-tunneled TCP.** That is most agent egress (model APIs, web, git/pip/npm, cloud SDKs), plus any non-HTTP TCP the client tunnels through the same proxy (databases, SSH, custom protocols), including IP-literal dials matched against CIDR policy rules. Same routing lock, same `observed` evidence.
- **Proxy-unaware tools fail closed**, not open. It's an availability cost I'm paying deliberately until transparent interception lands. Kernel-level syscall/file observation is future work, and file/tool governance today is cooperative, labeled `self-reported`, exactly as the model demands.
